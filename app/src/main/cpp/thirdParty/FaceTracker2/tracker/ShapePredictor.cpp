...
namespace  FACETRACKER
{
    ...
    ShapePredictor& ShapePredictor::operator= (ShapePredictor const&rhs)
    {
      ...
      _pdm = rhs._pdm;
      _warp = rhs._warp;
      SP_C.resize(_K); SP_R.resize(_K);
      for(int i = 0; i < _K; i++){
        SP_C[i] = rhs.SP_C[i].clone();
        SP_R[i] = rhs.SP_R[i].clone();
      }
...
//==============================================================================
    ...
    {
      s >> _K; SP_C.resize(_K); SP_R.resize(_K);
      for(int i = 0; i < _K; i++){
        FACETRACKER::IO::ReadMat(s,SP_C[i]);
        FACETRACKER::IO::ReadMat(s,SP_R[i]);
      }
...
//==============================================================================
    void ShapePredictor::ReadBinary(ifstream &s, bool readType)
    {
      ...
      SP_C.resize(_K); SP_R.resize(_K);
      for(int i = 0; i < _K; i++){
        FACETRACKER::IOBinary::ReadMat(s,SP_C[i]);
        FACETRACKER::IOBinary::ReadMat(s,SP_R[i]);
      }
...
//==============================================================================
    void ShapePredictor::Write(ofstream &s, bool binary)
    {
      if(!binary){
        s << _K << " ";
        for(int i = 0; i < _K; i++){
          FACETRACKER::IO::WriteMat(s,SP_C[i]);
          FACETRACKER::IO::WriteMat(s,SP_R[i]);
        }
        ...
        for(int i = 0; i < _K; i++){
          FACETRACKER::IOBinary::WriteMat(s,SP_C[i]);
          FACETRACKER::IOBinary::WriteMat(s,SP_R[i]);
        }
...
//==============================================================================
    int ShapePredictor::FindCluster(cv::Mat &shape)
    {
      ...
      for(int k = 0; k < _K; k++){
        FACETRACKER::CalcSimT(SP_C[k],z_,a1,b1,tx1,ty1);
